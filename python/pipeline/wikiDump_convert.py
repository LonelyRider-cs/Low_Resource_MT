...
def main(save_to_file):
    ...
    for root_file in os.listdir(location+"/../../all_wikipedia_dumps/text"):
        #loop through all wiki dump files in /text subdirectory
        for file in os.listdir(location+"/../../all_wikipedia_dumps/text/"+root_file):
            try:
                #if file is found say so in command line and proceed to convert json data
                if file.startswith("wiki_"):
                    print("wiki dump file found:\t" + str(file))
                    temp_json_file = "../../all_wikipedia_dumps/text/"+root_file+"/"+file
...
if __name__ == '__main__':
    ...
    print(today)
    print(sys.argv[1][:-8]+"_"+today+".txt")
    save_to_file = sys.argv[1][:-8]+"_"+today+".txt"
